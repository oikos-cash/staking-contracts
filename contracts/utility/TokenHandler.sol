...
contract TokenHandler {
    ...
    function _safeApprove(address _token, address _spender, uint256 _value) internal {
        _execute(_token, abi.encodeWithSelector(APPROVE_FUNC_SELECTOR, _spender, _value));
    }

    function _safeTransfer(address _token, address _to, uint256 _value) internal {
        _execute(_token, abi.encodeWithSelector(TRANSFER_FUNC_SELECTOR, _to, _value));
    }

    function _safeTransferFrom(address _token, address _from, address _to, uint256 _value) internal {
        _execute(_token, abi.encodeWithSelector(TRANSFER_FROM_FUNC_SELECTOR, _from, _to, _value));
    }

    function _execute(address _token, bytes memory _data) private {
        ...
        require(ret[0] != 0, "TokenHandler: ERR_TRANSFER_FAILED");
    }
}